    ...
    try {
      ...
      if (!membersSnapshot.empty) {
        ...
        membersSnapshot.forEach((doc) => {
          membersData.push({ ...doc.data(), id: parseInt(doc.id) });
        });
        setMembers(membersData.sort((a, b) => a.id - b.id));
      }
      ...
      if (!transactionsSnapshot.empty) {
        ...
        transactionsSnapshot.forEach((doc) => {
          transactionsData.push({ ...doc.data(), id: parseInt(doc.id) });
        });
        setTransactions(transactionsData.sort((a, b) => b.id - a.id)); // 최신순 정렬
      }

      // 예산 정보 로드
      const budgetDoc = await getDocs(collection(db, 'config'));
  ...
  const saveToCloud = async (dataType, data) => {
    ...
    try {
      ...
      if (dataType === 'members') {
        ...
        // 예산 정보 저장
        await setDoc(doc(db, 'config', 'budget'), data);
      }
